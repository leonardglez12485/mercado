...
export class DepartamentoService {
  ...
  async create(createDepartamentoDto: CreateDepartamentoDto) {
    createDepartamentoDto.nombre = createDepartamentoDto.nombre.toLocaleLowerCase();
    try {
  ...
  async findOne(term: string) {
    ...
    //busqueda por nombre
    if (!depto) {
      depto = await this.dptoModel.findOne({ nombre: term.toLocaleLowerCase().trim() });
    }
    if (!depto) {
  ...
  async remove(term: string) {
    ...
    if(deletedCount ===0){
      throw new BadRequestException(`Departamento whit ID: ${term} not Found !!!!`);
    }
  }
  ...
  private hadleException(error: any) {
    if (error.code === 11000) {
      throw new BadRequestException(`Mercado exists in DB ${JSON.stringify(error.keyValue)}`);
    }
    console.log({ error });
